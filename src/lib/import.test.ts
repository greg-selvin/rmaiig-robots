...
describe("source import",()=>{
  ...
  it("accepts blank robots and resolves country names and ISO codes to alpha-3",()=>{expect(parseRobotNames("")).toEqual({names:[],warning:null});expect(countryCode("USA")).toBe("USA");expect(countryCode("US")).toBe("USA");expect(countryCode("United States")).toBe("USA");expect(countryCode("UK")).toBe("GBR");expect(countryCode("GER")).toBe("DEU");expect(countryCode("XYZ")).toBeNull();});
});
