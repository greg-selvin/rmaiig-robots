...
async function processResearch(request:Request) {
  ...
  for(const job of jobs||[]) {
    ...
    try {
      ...
      await db.from("vendors").update({website_url:officialWebsite||vendor.website_url,iso_country_code:parsed.country_code||vendor.iso_country_code,research_status:"completed",last_researched_at:new Date().toISOString()}).eq("id",vendor.id);
      for(const location of parsed.locations){
        await db.from("vendor_locations").insert({workspace_id:workspaceId,vendor_id:vendor.id,location_type:location.type,country:location.country_code,iso_country_code:location.country_code,us_state_code:location.state_code,city:location.city,source_url:location.source_url,verified_at:new Date().toISOString()});
        if(location.type==="headquarters"&&location.country_code==="US")await db.from("vendors").update({us_state_code:location.state_code,headquarters_city:location.city}).eq("id",vendor.id);
      }
